...
run_test(){
  # Run the epm deploy
  echo ""
  echo -e "Testing eris-db using fixture =>\t$1"
  goto_base
  cd $1
  if [ "$ci" = false ]
  then
    ...
    $repo/eris-client tx send --node-addr "tcp://$chain_ip:46657" --sign-addr "http://$keys_ip:4767" --chain-id "$chain_name" --amt 100 --addr "$key1_addr" --to "$key2_addr" --debug
  else
    echo
    cat readme.md
    echo
...
echo "Hello! I'm the marmot that tests the eris-db node."
echo
start=`pwd`
cd $repo
# test_setup
...
then
  ...
  go build -o $repo/eris-client ./client/cmd/eris-client
  if [ $? -ne 0 ]
  then
    echo "Could not build eris-db. Debug via by directly running [`pwd`/tests/build_outside_tool.sh]"
    exit 1
  fi
  set +e
  echo "Build complete."
